...
def filter_csv(filename:str, column:str, value:float):
    ...
    """
    data_file = pd.read_csv(filename)
    data_file = data_file[data_file[column] <= value] # & data_file[column] > 60 
    result = data_file.to_json(orient="split", index=False)

    return result

def func_genre(filename:str, column:str, tipo:str):
    
    data_file = pd.read_csv(filename)
    data_file = data_file.loc[data_file[column] == tipo]  
    result = data_file.to_json(orient="split", index=False)
